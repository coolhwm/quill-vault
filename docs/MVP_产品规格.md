...
- 核心链路完整，可交付内测用户长期使用；
- 录音、逐字稿和已发布纪要不存在已知不可逆损坏路径；
- 逐字稿一旦就绪，纪要最终可通过继续或重新生成获得；
...
录音页采用专注的全屏流程。会议详情统一承载总结、图示、逐字稿和录音，不把同一会议拆成多个一级页面。BYOK 不占用独立 Tab。
...
flowchart LR
    ...
    RECORDING --> FINALIZING["正在整理逐字稿"]
    FINALIZING --> READY["待生成纪要"]
    READY --> GENERATING["正在生成纪要"]
    GENERATING --> COMPLETED["纪要已完成"]
```

1. 用户从首页或 Action Button 启动面对面会话；
2. App 立即开始录音，并在前台显示实时逐字稿；
3. 用户结束录音后，App 从原始录音补齐并原子写入完整逐字稿；
4. 会议进入“待生成纪要”；
5. 自动生成开启且模型可用时进入队列，否则等待用户手动生成；
6. 用户可以离开生成页，继续查看逐字稿或播放录音；
7. 纪要成功原子写入后显示“纪要已完成”。
...
- 首页首要视觉和交互元素是“开始录音”；
- 首次录音前完成权限、录音告知和存储空间检查；
- 点击后立即进入明确的录音状态，不能先等待 AI 或网络；
- 录音页显示时长、录音状态、音频中断、实时逐字稿和结束操作；
...
- 原始录音是转写的可追溯输入；
- 前台显示 volatile 与 final 结果，但只将 final 结果写入最终逐字稿；
- 后台无法持续分析时，恢复前台或结束后从录音追平；
- 去重、重叠处理和时间范围规范化必须确定；
- “逐字稿已就绪”只表示完整逐字稿已经原子写入权威目录；
- 已就绪逐字稿是纪要任务的不可变版本输入。

### 5.3 纪要管理

- 默认按最近时间排序；
- 支持按标题、摘要和逐字稿正文进行设备端全文搜索；
- 支持按日期、纪要状态和模型配置筛选；
- 会议卡片展示标题、时间、时长、状态和模型；
- 详情页支持总结、图示、逐字稿和录音四类内容；
- 逐字稿时间范围可定位录音播放位置；
...
- 首次启用自动生成时明确披露逐字稿会发送到目标域名并可能产生费用；
...
- 任务开始时绑定逐字稿内容指纹；
- 逐字稿被外部修改后，现有纪要标记“纪要已过期”；
- 基于旧逐字稿生成的结果仍可查看，但不能冒充最新；
- 重新生成基于最新逐字稿创建新任务。
...
- 记录模型名和服务域名，不记录逐字稿、纪要正文、API Key 或认证 Header；
