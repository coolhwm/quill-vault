...
MVP 必须重新建设，不能基于 Demo 工程继续开发。录音、文字记录和纪要不能因数据库、AI 或 UI 故障发生不可逆损坏；文字记录一旦就绪，最终纪要必须始终能够通过继续或重新生成获得。
...
会议资产继续以 `recording.m4a`、`transcript.md` 和 `minutes.md` 作为内容真相；SQLite/GRDB 只保存可重建索引、全文检索、模型配置元数据、诊断事件和生成任务检查点。生成任务绑定文字记录版本与模型配置快照，使用确定性步骤、幂等检查点、原子发布和可解释状态，确保任何中断都不会损坏已有资产。
